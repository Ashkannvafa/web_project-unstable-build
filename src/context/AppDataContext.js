...
export const AppDataProvider = ({ children }) => {
  ...
  const [language, setLanguage] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('harmoniq_language');
      return saved || 'en';
    }
    return 'en';
  });
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('harmoniq_language', language);
    }
  }, [language]);
  ...
  const [theme, setTheme] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('harmoniq_theme') || 'dark';
    }
    return 'dark';
  });
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      localStorage.setItem('harmoniq_theme', theme);
    }
  }, [theme]);
